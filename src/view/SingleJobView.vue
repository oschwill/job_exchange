...
onMounted(async () => {
  try {
    ...
    if (!state.job || state.job.length === 0) {
      router.push('/'); // Zur Homepage weiterleiten
    }
  } catch (error) {
...
<template>
  ...
  <section class="bg-green-50">
    <div v-if="!state.isLoading && state.job" class="container m-auto py-10 px-6">
    ...
    <div v-else class="text-center text-gray-500 py-6 h-screen flex justify-center items-center">
      <PulseLoader />
    </div>
  </section>
</template>
